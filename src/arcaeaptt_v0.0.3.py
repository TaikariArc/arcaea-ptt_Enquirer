...
# ========== 单曲 PTT 公式 ==========
def calculate_single_ptt(chart_constant, score, clear_bonus=0.0):
    if score >= 10000000:
        ptt = chart_constant + 2.0
    elif score >= 9800000:
        ptt = chart_constant + 1.0 + (score - 9800000) / 200000.0
    else:
        ptt = chart_constant + (score - 9500000) / 300000.0
        return max(0.0, ptt + clear_bonus)
    return ptt + clear_bonus

# ========== 通关奖励 ==========
def clear_bonus_from_ct(ct):
    """
    ct: 0=TL, 1=TC, 2=FR, 3=PM, 4=Easy, 5=Hard
    规则：PM/FR/TC/EasyClear +0.200；TL +0.000
    """
    if ct is None:
        return 0.0
    try:
...
def ct_label(ct):
    ...
    return full.get(ct, f'ct{ct}')

# ========== 主程序 ==========
def main():
    parser = argparse.ArgumentParser(description="计算 Arcaea PTT (B50 + B10)")
    ...
    # ---- 读 cleartypes 作为 fallback ----
    clear_map = {}
    try:
        cursor.execute("SELECT songId, songDifficulty, clearType FROM cleartypes")
        for sid, diff, ctype in cursor.fetchall():
            clear_map[(sid, diff)] = ctype
        print(f"cleartypes 记录数: {len(clear_map)}")
    except Exception as e:
        print(f"读取 cleartypes 失败（忽略）：{e}")

    # ---- 确认 scores 列 ----
    cursor.execute("PRAGMA table_info(scores)")
    columns = [col[1] for col in cursor.fetchall()]
    if 'songId' not in columns or 'songDifficulty' not in columns or 'score' not in columns:
        print(f"错误：scores 表缺少必要列。实际列: {columns}")
        sys.exit(1)
    has_ct = 'ct' in columns

    # ---- 取每个谱面最高分（连同 ct）----
    if has_ct:
        query = """
            SELECT songId, songDifficulty, score, ct
            FROM (
                SELECT songId, songDifficulty, score, ct,
                       ROW_NUMBER() OVER (
                           PARTITION BY songId, songDifficulty
                           ORDER BY score DESC
                       ) AS rn
                FROM scores
            )
        ...
        query = """
            SELECT songId, songDifficulty, score, 0 AS ct
            FROM (
                SELECT songId, songDifficulty, score,
                       ROW_NUMBER() OVER (
                           PARTITION BY songId, songDifficulty
                           ORDER BY score DESC
                       ) AS rn
    ...
    for song_id, difficulty, score, ct in rows:
        if score is None:
            continue

        # ct 优先；ct=0 时 fallback 到 cleartypes
        effective_ct = ct
        if effective_ct == 0:
            ct_zero_count += 1
            fb = clear_map.get((song_id, difficulty), 0)
            if fb != 0:
                effective_ct = fb
                ct_fallback_count += 1

        bonus = clear_bonus_from_ct(effective_ct)
        ...
        ptt = calculate_single_ptt(constant, score, bonus)
        rec = {
            'db_song_id': song_id,
            'json_key': jkey,
    ...
    print(f"ct=0（TL）条数: {ct_zero_count}")
    print(f"其中通过 cleartypes 找回非 TL 的条数: {ct_fallback_count}")
    print(f"回退匹配（BYD/ETR 混用）: {len(fallback_list)} 条")

    if args.debug or unmatched_song or unmatched_diff:
        print("\n匹配详情：")
        for r in results:
            tag = " [回退]" if r['is_fallback'] else ""
            print(f"  {r['db_song_id']:<25} -> {r['json_key']:<40} | "
                  f"难度 {r['difficulty']} ({r['diff_name']}) | 用键 \"{r['used_key']}\" | "
                  f"定数 {r['constant']} | 分数 {r['score']} | {ct_label(r['ct'])}(bonus={r['bonus']}){tag}")
    ...
    total_ptt = (b50_sum + b10_sum) / 60.0
    ...
    print("\n前 10 成绩：")
    print(f"{'排名':<4} {'PTT':<9} {'定数':<6} {'分数':<10} {'通关':<16} {'歌曲':<40} {'难度'}")
    for i, r in enumerate(best_10, 1):
        print(f"{i:<4} {r['ptt']:<9.4f} {r['constant']:<6.1f} {r['score']:<10} {ct_label(r['ct']):<16} {r['json_key']:<40} {r['diff_name']}")

    print("\nBest 50 列表：")
    for i, r in enumerate(best_50, 1):
        print(f"{i:<3} PTT: {r['ptt']:.4f} | 定数: {r['constant']:.1f} | 分数: {r['score']} | 通关: {ct_label(r['ct'])} | 歌曲: {r['json_key']} | 难度: {r['diff_name']}")
